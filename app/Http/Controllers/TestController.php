...
class TestController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        $tiles = $tiles->map(function(Hex $hex){
                ...
                return [
                    'q' => $q,
                    'r' => $r,
                    'ls' => $hex->security_level ?? rand(1, 3),
                    'type' => $hex->type,
                    'hasKey' => $hex->has_key,
                    'hasBox' => $hex->has_box,
    ...
    private function process(Collection $tiles): Collection
    {
        ...
        foreach($startingHexes as $startingHex){
            $adjacent = $tiles->filter(function(Hex $hex) use($startingHex){
                    return Hex::distance($startingHex->coordinates, $hex->coordinates) == 1 && !$hex->generated;
                })->random();

            $type = collect(['combat', 'ability', 'sociality', 'rest'])->random();
            $adjacent->fill([
                'type' => $type,
                'generated' => true,
                'resolved' => $type === 'rest'
